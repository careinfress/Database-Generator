...
public class ${entity.name.proc} {
    ...
    private static final ParamDef PARAM_DEF = ${entity.name.def}.Protocol.getParamDef(${entity.name.def}.Protocol.Def.${entity.name.def.underscoreUpper});
    protected static final String TABLE_NAME = "${table.name}";

    public ${entity.name.proc}(DaoPool daoPool, RedisCacheManager rdsCache, PosReadWriteLock lock) {
        m_daoPool = daoPool;
        m_rdsCache = rdsCache;
        m_lock = lock;
    }
    ...
    public int processAdd(FaiSession session) throws IOException {
        ...
        try {
            ...
            try {
                ...
                sltArg.searchArg.matcher = new ParamMatcher(${entity.name.def}.Info.AID, ParamMatcher.EQ, aid);
                ...
                try {
                    ...
                    if (rt == Errno.OK && m_rdsCache.exists(getCacheKey(aid))) {
                        ...
                        if (dataInfo != null) {
                            // 存入缓存
                            m_rdsCache.hsetParam(getCacheKey(aid), id + "", dataInfo, ${entity.name.def}.Protocol.Key.INFO_LIST, PARAM_DEF);
                        }
                    }
                } finally {
                    dao.close();
    ...
    public int processSet(FaiSession session) throws IOException {
        ...
        try {
            ...
            if (rt != Errno.OK || keyRef.value != ${entity.name.def}.Protocol.Key.ID || idRef == null) {
                rt = Errno.ARGS_ERROR;
            ...
            try {
                ...
                Dao.SelectArg sltArg = new Dao.SelectArg();
                sltArg.searchArg.matcher = new ParamMatcher(${entity.name.def}.Info.AID, ParamMatcher.EQ, aid);
                sltArg.searchArg.matcher.and(${entity.name.def}.Info.ID, ParamMatcher.EQ, id);
                sltArg.searchArg.matcher.and(${entity.name.def}.Info.DEL, ParamMatcher.EQ, 0);
                sltArg.table = TABLE_NAME;
                oldInfo = dao.selectFirst(sltArg);
                if (oldInfo == null) {
                    rt = Errno.DAO_SQL_ERROR;
                ...
                // 修改数据
                ParamMatcher matcher = new ParamMatcher(${entity.name.def}.Info.AID, ParamMatcher.EQ, aid);
                matcher.and(${entity.name.def}.Info.ID, ParamMatcher.EQ, id);
                matcher.and(${entity.name.def}.Info.DEL, ParamMatcher.EQ, 0);
                ...
                if (updater != null) {
                    // 修改缓存
                    m_rdsCache.hsetParam(getCacheKey(aid), id + "", updater, ${entity.name.def}.Protocol.Key.INFO_LIST, PARAM_DEF);
                }
            } finally {
                dao.close();
    ...
    public int processGet(FaiSession session) throws IOException {
        ...
        try {
            ...
            // 从缓存拿数据
            Param info = m_rdsCache.hgetParam(key, id, ${entity.name.def}.Protocol.Key.INFO_LIST, PARAM_DEF);
            if (info == null) {
                info = new Param();
            }
            ...
            info.toBuffer(sendBuf, ${entity.name.def}.Protocol.Key.INFO, PARAM_DEF);
    ...
    public int processGetList(FaiSession session) throws IOException {
        ...
        try {
            ...
            if (rt != Errno.OK || keyRef.value != ${entity.name.def}.Protocol.Key.FIELD) {
                rt = Errno.ARGS_ERROR;
            ...
            if (field != null && !field.isEmpty()) {
                ...
                list = m_rdsCache.hgetAllFaiList(key, ${entity.name.def}.Protocol.Key.INFO_LIST, PARAM_DEF);
                // 根据条件搜索
            ...
            // send info
            FaiBuffer sendBuf = new FaiBuffer(true);
            list.toBuffer(sendBuf, ${entity.name.def}.Protocol.Key.INFO_LIST, PARAM_DEF);
    ...
    private int initDataList(int flow, int aid) {
        ...
        try {
            ...
            searchArg.matcher = new ParamMatcher(${entity.name.def}.Info.AID, ParamMatcher.EQ, aid);
            Dao.SelectArg sltArg = new Dao.SelectArg();
            sltArg.table = TABLE_NAME;
            sltArg.searchArg = searchArg;
            list = dao.select(sltArg);
        ...
        // 存入缓存
        String key = getCacheKey(aid);
        m_rdsCache.hmsetFaiList(key, ${entity.name.def}.Info.ID, Var.Type.INT, list, ${entity.name.def}.Protocol.Key.INFO_LIST, PARAM_DEF);
